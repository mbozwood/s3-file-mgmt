            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <table class="shadow-lg bg-white table-auto" style="width: 100%">
                    <tr>
                        ...
                        <th class="bg-blue-100 border text-left px-8 py-4">Downloads</th>
                        <th class="bg-blue-100 border text-left px-8 py-4">Actions</th>
                    </tr>
                    @forelse($files as $file)
                        <tr>
                            <td class="border px-8 py-4">{{ $file->original_filename }}</td>
                            <td class="border px-8 py-4">{{ $file->user->name }}</td>
                            <td class="border px-8 py-4">{{ norm_date($file->created_at) }}</td>
                            <td class="border px-8 py-4">{{ $file->download_count }}</td>
                            <td class="border px-8 py-4">
                                <a href="{{ route('admin.show', $file) }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center">
                                    <svg class="fill-current w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M13 8V2H7v6H2l8 8 8-8h-5zM0 18h20v2H0v-2z"/></svg>
                                    <span>Download</span>
                                </a>
                                <form method="POST" action="{{ route('admin.destroy', $file) }}" class="inline">
                                    @method('DELETE')
                                    @csrf
